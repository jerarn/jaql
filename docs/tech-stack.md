...
Repository workspace files live in `.vscode/` (settings, tasks, launch configs). See
[docs/dev/vscode-workflow.md](dev/vscode-workflow.md) for the daily loop.

| Tool      | C++ extensions |
|-----------|----------------|
| **Cursor** | `anysphere.cpptools` only — auto-installs clangd, CMake Tools, and CodeLLDB as companion extensions |
| **VS Code** | `llvm-vs-code-extensions.vscode-clangd`, `ms-vscode.cmake-tools`, `ms-vscode.cpptools` |
| **CLion** | Built-in CMake and Conan integration (CLion 2024.1+) |

`.vscode/extensions.json` lists all of the above so each editor installs what it can;
Cursor ignores unavailable IDs, and VS Code ignores `anysphere.cpptools`.

### Language Server

[clangd](https://clangd.llvm.org/) is the language server for VS Code, Cursor, and any
LSP-compatible editor. Cursor's `anysphere.cpptools` bundles clangd and does not use
Microsoft IntelliSense. VS Code users install the clangd extension directly and should keep
`C_Cpp.intelliSenseEngine` disabled (already set in `.vscode/settings.json`) when both
cpptools and clangd are present.

clangd reads `compile_commands.json` generated by CMake
(`CMAKE_EXPORT_COMPILE_COMMANDS=ON` is set in all presets). Run `./scripts/bootstrap.sh`
before expecting full IntelliSense.
